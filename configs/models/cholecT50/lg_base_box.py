...
val_evaluator = dict(
    ...
    ann_file=os.path.join(data_root, 'val/annotation_ds_coco.json'),
    metric=['bbox'],
...
test_evaluator = dict(
    ...
    ann_file=os.path.join(data_root, 'test/annotation_ds_coco.json'),
    metric=['bbox'],
    additional_metrics=['reconstruction'],
    use_pred_boxes_recon=False,
...
default_hooks = dict(
    checkpoint=dict(save_best='cholecT50/bbox_mAP'),
)
