...
"""
Sourced from https://github.com/google/sg2im/blob/master/sg2im/layers.py
"""
...
def build_mlp(dim_list, activation='relu', batch_norm='none', dropout=0,
    ...
    for i in range(len(dim_list) - 1):
        ...
        if not final_layer or final_nonlinearity:
            if not isinstance(batch_norm, str) and batch_norm is not None:
                layers.append(batch_norm(dim_out))
            else:
                if batch_norm == 'batch':
...
def build_mask_net(dim, mask_size, scale_factor=2, output_dim=1, batch_norm=True):
    ...
    while cur_size < mask_size:
        ...
        layers.append(nn.ReLU())
        cur_size *= scale_factor
    if cur_size != mask_size:
        raise ValueError('Mask size must be a power of {}'.format(scale_factor))
    layers.append(nn.Conv2d(dim, output_dim, kernel_size=1))

    return nn.Sequential(*layers)

class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000, batch_first=False, return_enc_only=False):
        super(PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.batch_first = batch_first
        self.return_enc_only = False
    ...
    def forward(self, x):
        if self.batch_first:
            ...
            enc = self.pe[:x.size(0)]

        if self.return_enc_only:
