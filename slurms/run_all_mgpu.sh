...
# test
export CUDA_VISIBLE_DEVICES=0 && python $MMDETECTION/tools/test.py $(ls work_dirs/mt_${detector}/best_${dataset}* | tail -1) --work-dir ./work_dirs/mt_${detector} &
export CUDA_VISIBLE_DEVICES=1 && python $MMDETECTION/tools/test.py $(ls work_dirs/mt_${detector}_with_recon/best_${dataset}* | tail -1) --work-dir ./work_dirs/mt_${detector}_with_recon &
wait
