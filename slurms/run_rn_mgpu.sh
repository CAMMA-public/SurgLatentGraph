...
# test
export CUDA_VISIBLE_DEVICES=0 && python $MMDETECTION/tools/test.py $(ls work_dirs/simple_cvs_classifier/best_${dataset}* | tail -1) &
export CUDA_VISIBLE_DEVICES=1 && python $MMDETECTION/tools/test.py $(ls work_dirs/simple_cvs_classifier_with_recon/best_${dataset}* | tail -1) &
wait
