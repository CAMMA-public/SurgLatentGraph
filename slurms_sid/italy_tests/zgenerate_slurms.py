    ...
    for model in models:
        for dataset1 in datasets:
            for dataset2 in datasets:
                ...
                #WITHOUT RECON
                fname = model+"_"+detector+"_"+dataset1+"_"+dataset2+"_italy.sh"
                f = open(fname, "w")
                slurm = "#!/bin/bash \n#SBATCH -N 1 \n#SBATCH --cpus-per-task 10 \n#SBATCH --gres=gpu:1 \n#SBATCH --time=10:00:00 \n#SBATCH -p gpu_p13 \n#SBATCH -J "+model+ "_"+detector+"_"+dataset1+"_"+dataset2+"_italy \n#SBATCH --error "+model+ "_"+detector+"_"+dataset1+"_"+dataset2+"_italy_error.log \n#SBATCH --output " + model+ "_"+detector+"_"+dataset1+"_"+dataset2+"_italy.log \n#SBATCH -A lbw@v100 \n#SBATCH -C v100-32g \n\n\nmodule purge \nmodule load anaconda-py3/2019.03 \nmodule load gcc/9.3.0 \nmodule load cuda/10.2 \nexport LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/lib64 \nexport MMDETECTION=${WORK}/mmdet_files \nexport PYTHONPATH=${PYTHONPATH}:/gpfsscratch/rech/lbw/uou65jw/sid/latentgraph/ \n\n\ncd $SCRATCH/sid/latentgraph \nsource $(conda info --base)/bin/activate \nconda activate camma \n\n./slurms_sid/italy_tests/run_test.sh "+model+" "+detector+"_rcnn "+dataset1+" "+dataset2+" \n"
                f.write(slurm)
                f.close()
