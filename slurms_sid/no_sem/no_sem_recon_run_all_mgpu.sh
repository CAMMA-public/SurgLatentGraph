...
export CUDA_VISIBLE_DEVICES=0 && \
	python ${MMDETECTION}/tools/train.py ${cfg_dir}/lg_ds_${detector}_no_sem.py --work-dir  work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2} --cfg-options load_from=weights/${dataset}/lg_${detector}_no_recon.pth  && \
	# test on endoscapes
	cd configs/models/ && \
	./select_dataset.sh endoscapes && \
	cd ../.. && \
	python ${MMDETECTION}/tools/test.py ${cfg_dir}/lg_ds_${detector}_no_sem.py $(ls work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2}/best_${dataset2}* | tail -1) --work-dir work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2}/endoscapes && \
	# test on wc
	cd configs/models/ && \
	./select_dataset.sh wc && \
	cd ../.. && \
	python ${MMDETECTION}/tools/test.py ${cfg_dir}/lg_ds_${detector}_no_sem.py $(ls work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2}/best_${dataset2}* | tail -1) --work-dir work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2}/wc && \
	#copy everything back
	cp -r $JOBSCRATCH/latentgraph/work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2} $SCRATCH/sid/latentgraph/work_dirs/no_sem_yes_recon_lg_ds_${detector}_${dataset}_${dataset2} &
